...
export class AuthService {
  ...
  async checkAndRenewCredits(userId: string): Promise<{ renewed: boolean; bonusRenewed: boolean; newCredits: number; newBonusCredits: number }> {
    ...
    // Regular credits don't renew - they are just the initial 3 free credits
    // Only bonus credits renew every 2 days

    // Add 3 bonus credits every 2 days
    if (lastBonusRenewal < twoDaysAgo) {
      updateData.bonusCredits = sql`bonus_credits + 3`;
      updateData.lastBonusRenewal = now;
      bonusRenewed = true;
    }

    // Check if 14 days have passed since account creation - EXPIRE all bonus credits
    const lastBonusStart = user.lastBonusRenewal ? new Date(user.lastBonusRenewal) : new Date(user.createdAt!);
    
    if (lastBonusStart < fourteenDaysAgo && user.bonusCredits! > 0) {
      updateData.bonusCredits = 0; // Expire all bonus credits
      bonusRenewed = true;
    }
